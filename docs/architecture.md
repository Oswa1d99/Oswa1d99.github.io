...
- `src/components/RecordsView.astro`
- `src/components/RecordFilters.astro`
- `src/components/RecordList.astro`
- `src/pages/records/index.astro`

The Records Discovery Module owns page-local Records filtering, sorting, and shared listing orchestration. The pure content functions in `records-discovery.ts` define the filter and sort behavior, `RecordsView.astro` owns the common Records list frame and empty-state branch, and `RecordList.astro` emits a localized JSON payload for browser behavior.
